...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db),
    ...
    # post_dict = post.dict()
    # post_dict['id'] = randrange(start=1, stop=1000000)
    # my_post.append(post_dict)

    # cursor.execute("""INSERT INTO posts (title, content, published) VALUES (%s, %s, %s) RETURNING *""", (post.title,
    #                                                                                           post.content,
    #                                                                                           post.published))
    # new_post = cursor.fetchone()  # Getting the RETURNING VALUES from the above SQL query
    #
    # conn.commit()   # to save the newly inserted data into the actual database
...
def update_post(id: int, updated_post: schemas.PostCreate, db: Session = Depends(get_db),
    ...
    post_quer = db.query(models.Post).filter(models.Post.id == id)

    post = post_quer.first()
    ...
    post_quer.update(updated_post.dict(), synchronize_session=False)
    db.commit()
    # post_dict = post.dict()
    # post_dict['id'] = id
    # my_post[index] = post_dict
    return post_quer.first()
